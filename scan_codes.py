"""
uint16_t key values for the Teensy Arduino 'Keyboard.press(uint16_t keycode)'
interface.

Quote from Paul:
'''
The medium level press() and release() functions try to give
you a sort of Swiss Arm Knife style convenience. You can use the low level USB
key codes. Or you can give it ASCII and Unicode (up to U+C1FF) which get
translated to key codes depending on the keyboard layout (from the Tools >
Keyboard Layout menu).

The reason Keyboard.press() takes 16 bit input is it's trying to allow you to
give it any of these 7 types of input. Things are defined in this way to allow
as much Unicode range as possible and also automatically work with UTF8 packed
char constants. It's meant to "just work" in all the common ways people tend to
use it. The defines in keylayouts.h give 16 bit numbers where the lower 8 bits
are the actual USB HID usage numbers and the upper 8 bits are provided only for
the purpose of allowing Keyboard.press() to automatically (and hopefully
conveniently) figure out which type of input you meant.
'''

Constants taken from:
Arduino15\\packages\\teensy\\hardware\\avr\\1.59.0\\cores\\teensy4\\keylayouts.h
"""
